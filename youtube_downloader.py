...
def youtube_cmd(url_v):
    ...
    if max_v <= 1080:
        print(f"当前执行最大分辨率 {max_v} 尝试使用8个线程下载")
        subprocess.check_call(
            f'youtube-dl -f "bestvideo[height<=?{max_v}]+bestaudio/best" {url_v} --external-downloader aria2c --external-downloader-args "-x 8 -k 1M"', shell=True, cwd=path_exe)
    else:
        print(f"当前执行最大分辨率 {max_v} 尝试使用16个线程下载")
        subprocess.check_call(
            f'youtube-dl -f "bestvideo[height<=?{max_v}]+bestaudio/best" {url_v} --external-downloader aria2c --external-downloader-args "-x 16 -k 1M"', shell=True, cwd=path_exe)
